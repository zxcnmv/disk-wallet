<template>
    <div class="panel">
        ...
        <div class="list" v-else-if="$store.state.tokenList.length>0">
            <div class="item" v-for="(item,index) in editTokenMode? editList:$store.state.tokenList" 
            :key="index" :class="{'open':item.open&&!editTokenMode }">
            ...
            <div v-else>
                ...
                <div class="input-container" :class="{'has-error':inputErrorNotice!==''}">
                    <a-input v-model:value="transAmount" @change="transAmountChange" ref="transAmountInputEle"
                             :placeholder="account.input_amount+[transInfo.unit||transInfo.symbol]"/>
    ...
    <a-modal v-model:visible="showAddTokenPanel" :footer="null" :closable=false>
        <div class="base-token-list" v-if="baseTokenList">
            <div class="title-container">
                <div class="title">{{ account.token_list }}</div>
                <div class="action" @click="showAddCustomTokenPanel">{{ account.add_custom_token }}</div>
            </div>
            <a-input class="search-input" :placeholder="account.search_placeholder" type="text" v-model:value="inputSearch" @change="refreshSearch"></a-input>
            <div v-if="showTokenList && showTokenList.length>0">
                <div class="item" v-for="item in showTokenList" @click="addToken(item)" :key="item.genesis">
                    ...
                    <div class="info">
                        ...
                        <div class="genesis ellipsis">{{ popup.genesis }}: {{ item.genesis }}</div>
                    </div>
    ...
    methods: {
        ...
        async addCustomToken() {
            ...
            if (!result || result.code !== 0)
                return antMessage.error(this.account.token_error)
        ...
        transAmountChange(value) {
            ...
            if (this.transAmount > this.transBalance)
                return this.inputErrorNotice = this.account.balance_not_enough
            else
        ...
        async transfer() {
            ...
            if (bsvUtxoCount > 3 || utxoCount >= 20) {
                antMessage.warn(this.popup.merge_notice)
