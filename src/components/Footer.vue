<template>

    <div class="footer">
        <div class="line" v-if="false">
            <div style="display: flex">
                <a href="https://t.me/sensilet" target="_blank" rel="noopener">
                    <img src="/img/telegram.png" alt="">
                    <!--                <span class="word">-->
                    <!--                            Telegram-->
                    <!--                    </span>-->
                </a>
                <a href="https://github.com/brimless-lab/sensilet" target="_blank" rel="noopener">
                    <svg height="18" aria-hidden="true" viewBox="0 0 16 16" version="1.1" width="18" data-view-component="true" class="octicon octicon-mark-github v-align-middle">
                        <path fill-rule="evenodd"
                              d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"></path>
                    </svg>
                    <!--                <span class="word">-->
                    <!--                            GitHub-->
                    <!--                </span>-->
                </a>
                <a-dropdown placement="topCenter">
                    <div style="margin-left: 8px">
                        {{ 'lang'.$i18n.locale }}<DownOutlined/>
                    </div>
                    <template #overlay>
                        <a-menu>
                            <a-menu-item @click="chooseLanguage('en')">
                                <span>English</span>
                            </a-menu-item>
                            <a-menu-item v-if="false" @click="chooseLanguage('ja')">
                                <span>日本語</span>
                            </a-menu-item>
                        </a-menu>
                    </template>
                </a-dropdown>
            </div>
        </div>
        <div class="line" v-if="false">
            <a href="https://sensilet.com" class="copyright" target="_blank" rel="noopener">
                {{ setting.copyright }}
            </a>
            <a href="https://sensilet.com/privacy-policy.html" class="term" target="_blank" rel="noopener">
                {{ setting.privacy }}
            </a>
            <a href="https://sensilet.com/term-of-service.html" class="term" target="_blank" rel="noopener">
                {{ setting.term }}
            </a>
            <!--        <a href="https://sensilet.com" target="_blank" rel="noopener">-->
            <!--            <img src="../assets/logo.png" alt="">-->
            <!--            Sensilet-->
            <!--        </a>-->

        </div>
    </div>
</template>

<script>
import DownOutlined from '@ant-design/icons-vue/lib/icons/DownOutlined'
import messages from  '../language/index.js'
export default {
    name: "Footer",
    components:{DownOutlined},
    data(){
        return {
            wallet:messages.en.wallet,
            setting:messages.en.setting,
            account:messages.en.account,
        }
    },
    methods:{
        chooseLanguage(lang){
            localStorage.setItem('lang',lang)
            this.$i18n.locale = lang;
        }
    }
}
</script>

<style scoped lang="scss">

.footer {
    margin-top: 16px;
    height: 60px;

    background-color: #f5f7fa;

    color: #888;

    .line{
        display: flex;
        flex-direction: row;
        align-items: center;
        justify-content: center;
    }

    a {
        //margin: 0 16px;
        color: #888;
        display: flex;
        align-items: center;


        img, svg {
            border-radius: 50%;
            width: 18px;
            height: 18px;
            margin-right: 4px;
            margin-left: 8px;
        }

        .word {
            @media (max-width: 567px) {
                display: none;
            }
        }
    }

    .copyright, .term {
        margin: 8px;
    }

    .term {
        @media (max-width: 720px) {
            display: none;
        }
    }
}

</style>
