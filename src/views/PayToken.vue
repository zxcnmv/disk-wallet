<template>
    <div class="panel-container">
        <div class="panel scrolled">
            <div class="title" v-if="origin">{{ popup.pay_request.replace('{0}',['']) }}</div>
            <div class="pay-info" v-if="origin">
                <div class="origin">{{ origin }}</div>
            </div>
            <div class="title" v-else-if="tokenInfo">
                {{ popup.send_asset.replace('{0}',[tokenInfo.name]) }}
            </div>
            <div v-if="tokenInfo" style="margin-top: 20px">
                ...
                <div class="receive-container" :class="{'not-enough':tokenInfo.rest<0}" style="margin-top: 20px">
                    ...
                    <div class="receive-item" v-for="item in receivers" :key="item">
                        <div class="info">
                    ...
                    <div class="info">
                        <div class="key">{{ popup.balance_2 }}</div>
            ...
            <div class="action-container" v-if="!isPaying">
                ...
                <div>
                    <div class="info broadcast-info">
                        <div class="key">{{ popup.broadcast }}</div>
                        <div class="value">{{ broadcast ? popup.yes : popup.no }}</div>
                    </div>
                    <a-button type="primary" @click="commit">{{ popup.commit }}</a-button>
                </div>
            </div>
            <a-spin v-else/>
        </div>
...
export default {
    ...
    async mounted() {
        ...
        if (!tokenInfo) {
            antMessage.error('popup.unknown_token');
        ...
        async commit() {
            ...
            try {
                ...
                if (e && e.message && e.message.indexOf('Insufficient balance.') > -1) {
                    antMessage.error(this.popup.error_insufficient_balance)
                } else if (e && e.message && e.message.indexOf('Insufficient token') > -1) {
                    antMessage.error(this.popup.error_insufficient_token.replace('{0}',[this.tokenInfo.name]))
                } else if (e && e.message && e.message.indexOf('EC_REQ_FAILED') > -1) {
                    antMessage.error(this.popup.error_network + e.message)
                } else if (e)
