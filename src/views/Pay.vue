<template>
    <div class="panel-container">
        <div class="panel">
            <div class="title" v-if="origin">{{ popup.pay_request.replace('{0}',['BSV']) }}</div>
            <div class="pay-info" v-if="origin">
                <div class="origin">{{ origin }}</div>
            </div>
            <div class="title">
                {{ popup.send_asset.replace('{0}',['BSV']) }}
            </div>
...
export default {
    ...
    async mounted() {
        this.mineAddress = showLongString(await walletManager.getMainAddress())
        // let op = "";
        ...
        async commit() {
            try {
                if (this.total > this.balance) {
                    return antMessage.error('popup.error_balance')
                }
