<template>
    <div class="top-info">
        <div class="title">{{ wallet.private_key }}</div>
        <div class="notice">{{ wallet.private_notice_4 }}</div>
        <div class="warning">
...
<!--        <div class="notice"></div>-->
        <div class="seed-container">
            <div class="seed-title">{{ wallet.private_key }}</div>
        ...
        async next() {
            if (this.step === 0) {
                if (walletManager.isDefaultPassword()) {
                    ...
                    return antMessage.error(wallet.password_error_2)
                }
                this.mnemonic = await walletManager.getMainWif();
                //this.path = await walletManager.getPath() +'/0/0';

                clip = new Clipboard('#icon-copy');
                clip.on('success', e => {
                    antMessage.success(this.account.clip.replace('{0}',['privateKey']));
                });
            }
        },
        goBack() {
