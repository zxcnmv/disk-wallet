<template>
        ...
        <div>
            <div class="connect-container-line">
                <div class="connect-container" :class="{'connected':$store.state.isConnected}" @click="gotoConnectManagement">
                    ...
                    <div class="word">{{ $store.state.isConnected ? account.connected : account.not_connected }}</div>
                </div>
                <div class="connect-container" @click="openWeb">
                    <FullscreenOutlined class="item-icon"/>
                    <div class="word">{{ setting.expand_view }}</div>
                </div>
            ...
            <div class="panel" v-if="false">
                <div class="panel-top">
                    <div class="title"> {{ account.hot_app }}</div>
                </div>
                <div class="app-list" v-if="appList!=null && canShowApp">
                    <!--                <div class="app-list" v-if="false">-->
                    <div class="item" v-for="item in appList" :key="item">
            ...
            <div v-else>
                ...
                <div class="input-container" :class="{'has-error':inputErrorNotice!==''}">
                    <a-input v-model:value="transAmount" @change="transAmountChange" type="number" ref="transAmountInputEle"
                             :placeholder="account.input_amount.replace('{0}',[transType==='BSV'? 'BSV':(transInfo.unit||transInfo.symbol)])"/>
                ...
                <div class="notice">
                    <div class="balance" v-if="transInfo">
                        ...
                        <div class="action" @click="sendAll">{{ account.send_all }}</div>
                    </div>
                    <div class="fee">
                        <div class="key">
                            {{ 'account.fee' }}:
    ...
    <a-modal v-model:visible="showQr" :footer="null" :closable=false>
        <div style="font-size: 18px;text-align: center;margin-bottom: 16px">{{ account.qr_title }}</div>
...
export default {
    ...
    data() {
        return {
            ...
            canShowApp: true,
            transStep: 0,
            transStepNextWord: messages.en.account.next,
            transStepBackWord: messages.en.account.cancel,
            inputErrorNotice: "",
    ...
    async created() {
     ...
     mounted() {
        ...
        clip.on('success', e => {
            antMessage.success(this.account.clip.replace('{0}',[e.text]));
        });
    ...
    methods: {
        ...
        async initAsset() {
            this.bsvAsset.address = await walletManager.getMainAddress()
            this.mainAddress = await walletManager.getMainAddress()
        ...
        async onTransNext(address) {
            ...
            this.transStepNextWord = this.account.ok
            this.transStepBackWord = this.account.back
        ...
        transBack() {
            this.transStepNextWord = this.account.next
            this.transStepBackWord = this.account.cancel
        ...
        async sendAll() {
            let {amount, fee} = await walletManager.getSendAllInfo(await walletManager.getMainWif());
        ...
        transAmountChange(value) {
            ...
            if (this.transAmount * Math.pow(10, 8) > this.transInfo.balance.total)
                return this.inputErrorNotice = 'account.balance_not_enough'
            else
        ...
        async transfer() {
            ...
            if (!amount || isNaN(amount))
                return antMessage.error(this.account.amount_error)

            switch (this.transType) {
                case "BSV":
                    amount = Math.round(parseFloat(amount) * Math.pow(10, 8));
                    if (amount <= 0)
                        return antMessage.error(this.account.amount_error_2)
                    if (amount > this.transInfo.balance.total)
                        return antMessage.error(this.account.balance_not_enough)
                ...
                case "TOKEN":
                    ...
                    //     antMessage.warn('popup.merge_notice'))
