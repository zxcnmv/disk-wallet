                ...
                <a-dropdown placement="bottomCenter" v-if="account!=null"
                    ...
                    <template #overlay>
                        <a-menu v-if="addressList  && addressList .length>0" style="max-height: 60vh;overflow-y: scroll">
                            <a-menu-item v-for="item in addressList" @click="choose(item)" :key="item" class="account-item"
...
export default {
    ...
    methods: {
        ...
        disconnect(origin) {
            let _this = this;
            antModal.confirm({
                title: 'setting.disconnect'+ " " + origin,
                content: 'setting.disconnect_notice',
                async onOk() {
