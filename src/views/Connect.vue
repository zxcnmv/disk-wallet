<template>
    ...
    <div class="panel">
        <div class="title">{{ popup.info_request }}</div>
        <div class="link-info">
        ...
        <div class="notice">{{ popup.connect_notice }}</div>

    </div>
        <div class="action-panel">
            <div class="action-container" v-if="!isConnecting">
                <a-button @click="cancel">{{ popup.cancel }}</a-button>
                <a-button type="primary" @click="connect">{{ popup.connect }}</a-button>
            </div>
            <a-spin v-else/>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async connect() {
            ...
            let data = await connectManager.connect(address,origin).catch(e => console.log('e',e));
            //console.log(data)
            chrome.runtime.sendMessage({
                channel: 'sato_extension_background_channel_disk',
                data: {
