...
async function getLocalTokenList() {

    return await storageUtils.getItem('tokenList');
}

async function getLocalTokenTable() {

    let tokenList = await getLocalTokenList();
    let tokenTable = {};
...
async function getAllTokenTable() {
    let tokenTable = await localManager.getAllTokenTable();
    if (!tokenTable) {
        //    通过网络获取全部token信息
        tokenTable = {};
        ...
        if (data) {
            ...
            await localManager.setAllTokenTable(tokenTable, version)
...
tokenManager.getTokenListNet = async function () {
    ...
    let local = await getLocalTokenList();
    if (data) {
        let temp = {};
        ...
        await localManager.setAllTokenTable(allTokenTable, version)
    }
    // console.log(data)
...
tokenManager.refreshLocalTokenData = async function () {
    let localList = await getLocalTokenList();
...
tokenManager.reSaveToken = function (tokenList = []) {
    storageUtils.setItem('tokenList', tokenList)
}
tokenManager.addToken = async function (tokenInfo) {
...
tokenManager.listUserTokens = async function (showAll = false) {
    ...
    if (result.code === 0) {
        ...
        //查询接口返回错误时
        tokenList = await getLocalTokenList();
        for (let i = 0; i < tokenList.length; i++) {
...
tokenManager.fixPic = async function () {
    let list = await getLocalTokenList()
