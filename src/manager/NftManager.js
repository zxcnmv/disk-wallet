...
nftManager.getNftInfoNet = async function () {
    ...
    if (data && data.length > 0) {
        ...
        await localManager.setNftInfoTable(table, version)
...
nftManager.getNftInfoTable = async function () {
    let table = await localManager.getNftInfoTable();
    if (!table) {
...
nftManager.listAllNft = async function () {
    return sensibleNftUtils.getSummary(await walletManager.getMainAddress())
};
