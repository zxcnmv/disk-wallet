...
routerManager.goto = async function (url, data) {
    if (config.debug)
    ...
    if (pageConfig) {
        if (pageConfig.needCreate && !(await walletManager.isMnemonicCreate())) {
            needGoto = '/create'
        } else if (pageConfig.needUnlock && (await walletManager.isNeedUnlock())) {
            needGoto = '/unlock'
        }
    }
