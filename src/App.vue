...
<!--            <span v-if="debug">{{ currentPage }}</span>-->
    ...
    <div class="main">
        ...
        <SignPopAuthorize v-else-if="currentPage==='/signPopAuthorize'"/>
    </div>
...
const SignPopAuthorize = defineAsyncComponent(() => import( "./views/SignPopAuthorize"));
...
export default {
    ...
    watch:{
        currentPage(v,oldV){
            console.log('currentPage',v,oldV)
        }
    },
    beforeCreate() {
        ...
        if (request && request.method === 'connect') {
            console.log('connect to diskWallet');
        ...
        if (request && request.method === 'signPopAuthorize') {
            console.log('sign pop authorize');
            return routerManager.goto('/signPopAuthorize')
        }
